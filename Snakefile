...
rule fastqc_raw:
    ...
    output:
        r1 = "{config[path]}fastqc/{sample}_R1_001_fastqc.html",
        r2 = "{config[path]}fastqc/{sample}_R2_001_fastqc.html"
    conda: "envs/fastqc_env.yaml"
    shell: "fastqc -o {config[path]}fastqc {input.r1} {input.r2}"

rule multiqc_raw:
    input:
        r1 = expand("{config[path]}fastqc/{sample}_R1_001_fastqc.html", sample=SAMPLES),
        r2 = expand("{config[path]}fastqc/{sample}_R2_001_fastqc.html", sample=SAMPLES),
    output: "results/multiqc_report_raw.html"
    conda: "envs/multiqc_env.yaml"
    shell: "multiqc -f {config[path]}fastqc -o results -n multiqc_report_raw.html"
...
rule prinseq:
    input:
        ...
        r2 = "data/trimdata/{sample}_r2_trim.fastq.gz"
    output:
        "data/filtdata/{sample}_filtered"
    conda: "envs/prinseq_env.yaml"
    shell:
            "perl prinseq-lite.pl -fastq {input.r1} -fastq2 {input.r2} "
            "-out_good {output} -out_bad null -lc_method dust -lc_threshold 7 "
            "-derep 1"

rule fastqc_filt:
    input:
        r1 = "data/filtdata/{sample}_filtered_1.fastq.gz",
        r2 = "data/filtdata/{sample}_filtered_2.fastq.gz"
    output:
